...
つまりクラスが種類であり、その種類に含まれる1つ1つのデータがオブジェクト（インスタンス）
...
そして、サービスを作る上で足りない部分に関しては、自身で新しいクラス（オブジェクトの種類）を作成することで、
...
class User
  ...
  # tom.name = "John"  のようなことが可能になる .nameというsetterメソッド（インスタンス変数を変更する）
  # tom.name  => tom   のようなことが可能になる .nameというgetterメソッド（インスタンス変数を取得する）
